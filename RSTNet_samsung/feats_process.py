...
def process_dataset(file_path, feat_paths):
    print('save the ori grid X101-test2015 to the X101-test2015 with specified size')
    # 加载特征处理器
    ...
    with h5py.File(file_path, 'w') as f:
        for i in tqdm(range(len(feat_paths))):
            ...
            if feat_path[-4:] == '.npy':
                ########################################## load npy file
                img_feat = torch.from_numpy(np.load(feat_path).astype(float))
                # 处理特征
                ...
                f.create_dataset('%s_grids' % img_id, data=img_feat.numpy())
        f.close()


def get_feat_paths(dir_to_save_feats, data_split='trainval', test2014_info_path=None):
    print('get the paths of raw grid X101-test2015')
    ans = []
    # 线下训练和测试
    if data_split == 'trainval':
        filenames_train = os.listdir(os.path.join(dir_to_save_feats, 'train2014'))
        ans_train = [os.path.join(dir_to_save_feats, 'train2014', filename) for filename in filenames_train if filename]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dir_to_processed_feats', type=str, default='./Datasets/X101-features-little/')
