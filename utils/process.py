...
###########################################################
    ...
    l = len(object_data)
    train_name = object_data[:int(l * 0.9)]
    val_name = object_data[int(l * 0.9):]
    return train_name, val_name
######################################################################

def split_dataset_koniq10k_arg(txt_file_name, split_seed=20):
    np.random.seed(split_seed)
    object_data = []
    #    with open(txt_file_name, 'r') as listFile:
    ########################################################### append
    with open(txt_file_name, 'r') as col_names:
        ...
        for j in tqdm(train_name, desc='trainset'):
            tr += 1
            for i in range(5):
                x = i+1
                name = str(x)+'_'+j
        ...
        for j in tqdm(val_name, desc='test set'):
            val +=1
            for i in range(5):
                x = i+1
                name = str(x)+'_'+j
...
def split_dataset_koniq10k_0_9(txt_file_name, split_seed=20):
    np.random.seed(split_seed)
    object_data = []
    #    with open(txt_file_name, 'r') as listFile:
    ########################################################### append
    with open(txt_file_name, 'r') as col_names:
        reader_1 = csv.reader(col_names)
        listFile = next(reader_1)
        ###########################################################
...
######################################################################
def split_dataset_koniq10k_all(txt_file_name, split_seed=20):
    np.random.seed(split_seed)
    object_data = []
    #    with open(txt_file_name, 'r') as listFile:
    ########################################################### append
    ...
    l = len(object_data)
    train_name = object_data[:int(l)]
#    val_name = object_data[int(l * 0.9):]
    return train_name


def split_dataset_koniq10k_for_test(txt_file_name, split_seed=20):
    np.random.seed(split_seed)
    object_data = []
    #    with open(txt_file_name, 'r') as listFile:
    ########################################################### append
...
def split_dataset_koniq10k_training_test(txt_file_name, split_seed=20):
    np.random.seed(split_seed)
    object_data = []
    #    with open(txt_file_name, 'r') as listFile:
    ########################################################### append
...
class RandCrop_inference(object):
    def __init__(self, patch_size):
        self.patch_size = patch_size
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        return sample
##########################################################
class Normalize_infernece(object):
    def __init__(self, mean, var):
        self.mean = mean
        self.var = var
