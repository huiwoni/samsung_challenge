...
class MANIQA_5x5(nn.Module):
    def __init__(self, embed_dim=72, num_outputs=1, patch_size=8, drop=0.1,
        ...
        #################################################################
        self.conv3 = nn.Conv2d(2, 1, (5, 5), 1, 2)
    ...
    def forward(self, x):
        # Exit from vit and merge
        #########################################################################
        x_img = x[:, :3]
        x_HZ = x[:, 3:]
        _x = self.vit(x_img)
        x_img = self.extract_feature(self.save_output)
        self.save_output.outputs.clear()
        _y = self.vit(x_HZ)
        x_HZ = self.extract_feature(self.save_output)
        self.save_output.outputs.clear()
        ##########################################################################
        x = torch.stack([x_img, x_HZ], dim=1)  # Extracted feature X
        x = self.conv3(x)
        x = torch.squeeze(x)  # Embeded feature F
...
class MANIQA_3x3(nn.Module):
    def __init__(self, embed_dim=72, num_outputs=1, patch_size=8, drop=0.1,
        ...
        #################################################################
        self.conv3 = nn.Conv2d(2, 1, (3, 3), 1, 1)
